...
namespace ld53::input
{
    ...
    void sokol_capture_keyboard_events(bool enable) {
        ...
        window.call<void>(action, emscripten::val("keyup"), emscripten::val::module_property("event_keyup"));
        window.call<void>(action, emscripten::val("keydown"), emscripten::val::module_property("event_keydown"));
    }

    EMSCRIPTEN_BINDINGS(ld53) {
        emscripten::function("sokol_capture_keyboard_events", sokol_capture_keyboard_events);
